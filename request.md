...
스크랩된 내용은 kr_economy_news.md 파일의 하단에 추가되게 해줘.(기존 파일은 그대로 냅두고)
 ...
 - [x] 18. 증시정보 추가
 아래 증시정보를 네이버 금융 혹은 키움증권 API를 사용해서 가져와서 
 아래 샘플 구조대로 추가해. main_kr.py 실행할때만 추가.

 "
 증시정보

코스피 6,742.74(+0.68%), 거래대금 23.6조
코스닥 827.15(+1.70%), 거래대금 4.6조
코스피 200 1060.68(+0.63%)
...
​- 달러환율 1,385.10원(+1.10원)
- 일본JPY(100엔) 869.11원(-0.62원)
- 유럽연합EUR 1615.37원(+1.21원)
- 중국CNY 206.02원(+0.13원)
- 유가(WTI) 85.01(-2.05달러)
- 미국국채 10년 4.7080(-0.13%)
...
"
 ...
 - [x] 18. 매매동향 정보 네이버 수집에서 키움증권 API로 변경 개발
아래 매매동향 정보 수집해오는 부분 현재 네이버에서 가져오는 
키움증권 API로 수정개발. 기존 네이버 수집부문은 키움이 안될경우 우회로 처리하는 걸로 남겨둠.
REST API ka10051 사용하면 될듯.
금액수량구분은 금액으로 조회, 거래소는 통합으로 요청할 것. 
