...
1. **시황 수집**: `market.py` 또는 `market_kr.py`를 통해 지수 및 거래대금 상위 종목 수집 (키움 API NEXT / 네이버 금융). 미국 시장의 경우 yfinance 시계열 데이터 결측 시 개별 종목 정보(info) API를 활용해 가격 및 등락률을 복구하는 예외 처리 적용.
2. **뉴스 수집**:
