...
2. **뉴스 수집**: `scraper.py` / `scraper_kr.py`를 통해 하이브리드 수집 (구글 RSS 검색 최우선).
3. **데이터 정규화**: `decode_google_news_url`을 통한 URL 복원 및 제목 정규화로 중복 제거 기반 마련.
4. **정렬 알고리즘**: 검색 엔진 원본 랭킹(`original_rank`)과 발행 시간을 결합한 다차원 정렬 적용.
5. **저장**: `formatter.py`를 통해 마크다운 리포트 생성 및 저장.
